...
public abstract class AppLogger {
	...
	public static void error(String message, Class<?> clazz) { logger.error(getFormattedClassString(clazz) + message); }

	public static void trace(String message, Class<?> clazz) { logger.trace(getFormattedClassString(clazz) + message); }

	private static String getFormattedClassString(Class<?> clazz) { return "[" + clazz.getSimpleName() + "] "; }
}
